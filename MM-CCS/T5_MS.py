...
target_column = "Overall_Summary"  #For MCS, target_column = "One_line_patient_summary"
...
with torch.no_grad():
    for batch in test_loader:
        ...
        outputs = model.generate(input_ids=inputs, max_length=25)  # Adjust max_length as needed
        # Check if outputs is empty
        if len(outputs) == 0:
            print("Empty predictions for batch:", batch)
            continue
...
new = pd.DataFrame(list(zip(patient_dialogue, actual, dec_pred)),
...
score_filename = "T5_OS.p"
pickle.dump(scores, open(score_filename, 'wb'))

pred_filename = "T5_OS.p"
