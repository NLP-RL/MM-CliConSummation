...
def preprocess_dataset(dataset):
    ...
    model_inputs['intent_labels'] = torch.tensor(dataset["Group_mapped"], dtype=torch.long).to(DEVICE) 
...
class MultimodalBartForConditionalGeneration(BartPretrainedModel):
    ...
    def forward(
        ...
        if intent_labels is not None:
             ...
             # Total_loss = 0.7*classification_loss + 0.3*masked_lm_loss ## (Changess)
...
class MultimodalBartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.fusionVis_at_layer = [2]
        self.fusionKG_at_layer = [4]
        self.visual_transformer = TransformerEncoder(d_model=VISUAL_DIM, 
                                                     num_layers=3,
                                                     num_heads=8, 
                                                     dim_feedforward=VISUAL_DIM)
...
def val_epoch(model,
    ...
    with torch.no_grad():
        for step, batch in enumerate(tqdm(data_loader, desc="Validation Loss Iteration")):
            ...
            input_ids, attention_mask, visual_input, kg_input, labels, intent_labels = batch
...
def prepare_for_training(model,
    ...
    for name, param in model.named_parameters():
        if "kg_transformer" or "visual_transformer" or "MAF_layer" in name:
